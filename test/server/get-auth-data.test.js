...
test('get-auth-data >> No Signature flag in Authorization header', t => {
  ...
  reqStub.headers.authorization = 'timestamp=123 login=abc signature=def';
...
test('get-auth-data >> No login field in Authorization header', t => {
  ...
  reqStub.headers.authorization = 'Signature timestamp=123 signature=def';
...
test('get-auth-data >> No signature field in Authorization header', t => {
  ...
  reqStub.headers.authorization = 'Signature timestamp=123 login=abc';
...
test('get-auth-data >> No timestamp field in Authorization header', t => {
  const reqStub = getRequestStub();
...
test('get-auth-data >> Correct Authorization header', t => {
  ...
  reqStub.headers.authorization = 'Signature timestamp=123 login=abc signature=def';

  self(reqStub);

  t.notOk(reqStub.user.errorCode, 'No Error code');
  t.ok(reqStub.user.timestamp, 'There is timestamp');
